...
def load_speech_data(data_dir):
    song_list = os.listdir(data_dir)[0:60]
    spect_list = []
        
    for song in song_list:
        print(f"Loading song {song}")
        wave, sr = librosa.load(os.path.join(data_dir, song, 'vocals.wav'), duration=15)
...
def train_speech_bases(data_dir, num_bases, num_iterations, save_bases=True):
    M = load_speech_data(data_dir)
    
    B = cp.random.random((M.shape[0], 200))

    print("Running NMF")

    B, W = nmf(M, num_bases,n_iterations=num_iterations, starting_bases=B, verbose=True)
    if save_bases:
...
if __name__ == "__main__":
    data_dir = "../data/train"
    train_speech_bases(data_dir, 200, 200)
